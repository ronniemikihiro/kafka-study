...
import br.com.producer.controller.dto.PeopleDTO;
import br.com.producer.producer.PeopleProducer;
...
public class PeopleController {
    ...
    public ResponseEntity<Void> sendMessage(@RequestBody PeopleDTO peopleDTO) {
        ...
        var people = PeopleAvro.newBuilder()
                .setId(id)
                .setName(peopleDTO.getName())
                .setCpf(peopleDTO.getCpf())
                .setBooks(peopleDTO.getBooks().stream().map(CharSequence.class::cast).toList())
                .build();
